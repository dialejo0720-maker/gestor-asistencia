      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">Resumen de ventas por empleado</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-blue-600 text-white rounded-xl p-4 col-span-2 md:col-span-1">
            <p className="text-blue-200 text-xs">Total del día</p>
            <p className="text-2xl font-bold mt-1">${totalVentas.toLocaleString('es-CO')}</p>
            <p className="text-blue-200 text-xs mt-1">{sales.length} registros</p>
          </div>
              ...
              <tbody>
                {sales.map((s, i) => (
                  <tr key={s.id} className={`border-b last:border-0 ${i % 2 === 1 ? 'bg-gray-50/30' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-right font-bold text-green-600">${s.monto.toLocaleString('es-CO')}</td>
                  </tr>
                ))}
                {sales.length === 0 && (
                  <tr><td colSpan={4} className="text-center py-10 text-gray-400">No hay ventas para esta fecha</td></tr>
                )}
              </tbody>
            </table>
          )}
