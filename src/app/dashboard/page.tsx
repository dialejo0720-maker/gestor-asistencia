...
export default function DashboardPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [empRes, tsRes, salesRes] = await Promise.all([
      fetch('/api/employees'),
      fetch(`/api/dashboard/today?date=${today}`),
      fetch(`/api/sales?fecha=${today}`),
    ]);
    ...
    const totalV = (salesJson.sales || []).reduce((a: number, s: { monto: number }) => a + Number(s.monto), 0);
    setVentasHoy(totalV);
  }, []);
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="bg-green-600 rounded-xl p-4 text-white">
            <p className="text-green-200 text-xs mb-2">Ventas hoy</p>
            <p className="text-3xl font-bold">${ventasHoy.toLocaleString('es-CO')}</p>
            <a
              href={`https://wa.me/?text=${encodeURIComponent(`Ventas del día ${hoy}\nTotal: $${ventasHoy.toLocaleString('es-CO')}`)}`}
              target="_blank"
              className="text-xs bg-green-500 hover:bg-green-400 px-3 py-1.5 rounded-lg w-full transition mt-2 flex items-center justify-center gap-1"
            >
